...
mydb = mysql.connector.connect(host = "localhost", user = "mohzashraf", passwd="bachelorATM123!", database="atm")
...
def enter(request):
    ...
    if pinEntered.count('_') < 4:
        ...
        balance = viewsHome.getBalance(viewsPass.card_number)

        total = int(balance) + int(deposit)

        viewsHome.updateBalance(viewsPass.card_number, str(total))

        success = "AMOUNT IS ADDED TO YOUR BALANCE SUCCESSFULLY!"
        pinEntered = ['_','_','_','_'] 
        error = ""
        return returnRender(request)
